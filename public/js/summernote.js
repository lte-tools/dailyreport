...
(function (factory) {
  ...
  var Table = function () {
    ...
    this.createTable = function (nCol, nRow) {
      ...
      for (var idxCol = 0; idxCol < nCol; idxCol++) {
        aTD.push('<td style="border:solid black 1px">' + dom.blank + '</td>');
      }
      sTD = aTD.join('');
      ...
      sTR = aTR.join('');
      var sTable = '<table class="table table-bordered" border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse">' + sTR + '</table>';
      return $(sTable)[0];
    };
  };
  ...
  var Progress = function () {
    ...
    this.createProgress = function (ratio) {

      var yDom=$('<table class="barlist"></table>');
      yDom.append(
          $('<tr></tr>').append(

            $('<td></td>').append(
              $('<table id="pval" class="index"></table>').append(
                  $('<tr></tr>').append(
                      $('<td></td>').css({"height":"24px","background-color":"#428bca","margin":"0px","padding":"0px"}).append(
                        $('<p></p>').css({"height":"24px","background-color":"#428bca","text-align":"center","margin":"0px","padding":"0px"}).html(function(){
                  return ratio+"%";
                })
                        )
                    )
                ).css({"height":"24px","background-color":"#428bca","margin":"0px","padding":"0px"}).css("width",function(){
                  return ratio+"%";
                })
             ).css({"positon":"relative","list-style":"none","display":"block","width":"300px","height":"24px","margin":"2px","background-color":"#f5f5f5","padding":"0px"})
          

            )
        ).css({"position":"relative","width":"300px","list-style":"none","height":"24px"});


      /*
      yDom.append(
          $('<div id="pVal" class="progress-bar" role="progressbar"  aria-valuemin="0" aria-valuemax="100"></div>')
          .css("width",function(){
            return ratio+"%";
          }).css("min-width","18px").html(function(){
            return ratio+"%";
          })
  ...
  var Editor = function () {
    ...
    this.insertProgressBar = function ($editable, pVal) {
      recordUndo($editable);
      range.create().insertNode(progress.createProgress(pVal));
    }
  ...
  var Dialog = function () {
    ...
    this.showProgressDialog = function ($editable, $dialog, linkInfo) {
      return $.Deferred(function (deferred) {
        var $progressDialog = $dialog.find('.note-progress-dialog');
        $progressUrl = $progressDialog.find('.note-progress-url'),
        $progressBtn = $progressDialog.find('.note-progress-btn'),
        ...
        $progressDialog.one('shown.bs.modal', function (event) {
         ...
         // $openInNewWindow.prop('checked', linkInfo.newWindow);
          ...
          $progressBtn.one('click', function (event) {
            event.preventDefault();
            $progressDialog.modal('hide');
            deferred.resolve($progressUrl.val());
          });
        }).one('hidden.bs.modal', function (event) {
          event.stopPropagation();
          $progressBtn.toggleClass('disabled', false);
      ...
      }).promise();  //$.Deferred().promise() returns promise()obj
    };
  ...
  var EventHandler = function () {
    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      if ($btn.length > 0) {
        ...
        } else if(sEvent === 'showProgressDialog'){
          $editable.focus();
          var linkInfo = editor.getLinkInfo();

          editor.saveRange($editable);
          dialog.showProgressDialog($editable, $dialog, linkInfo).then(function (data) {
            editor.restoreRange($editable);
            editor.insertProgressBar($editable, data);
          });
  ...
  var Renderer = function () {
    ...
    tplDialog = function (lang, options) {
      ...
      var tplProgressDialog = function () {
        return '<div class="note-progress-dialog modal" aria-hidden="false">' +
                 '<div class="modal-dialog">' +
                   '<div class="modal-content">' +
                     '<div class="modal-header">' +
                       '<button type="button" class="close" aria-hidden="true" tabindex="-1">&times;</button>' +
                       '<h4>' + lang.progress.insert + '</h4>' +
                     '</div>' +
                     '<div class="modal-body">' +
                       '<div class="row-fluid">' +
                         '<div class="form-group">' +
                           '<label>' + lang.progress.url + '</label>' +
                           '<input class="note-progress-url form-control span12" type="text" />' +
                         '</div>' +
                       '</div>' +
                     '</div>' +
                     '<div class="modal-footer">' +
                        '<button href="#" class="btn btn-primary note-progress-btn disabled" disabled="disabled">' + lang.progress.insert + '</button>' +
                       //'<button type="button" class="btn btn-default" data-dismiss="modal">'+'Close'+'</button>'+
                       //'<button type="button" id="saveBtn" class="btn btn-primary" data-event="insertProgressBar">'+'Save changes'+'</button>'+
                     '</div>' +
                   '</div>' +
                 '</div>' +
               '</div>';
